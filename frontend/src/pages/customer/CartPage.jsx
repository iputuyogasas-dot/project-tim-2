...
import { ArrowLeft, ShoppingBag, Trash2, Plus, Minus, Utensils } from 'lucide-react';
            ...
            <div className="container" style={{ paddingBottom: '160px' }}>
                {cart.length === 0 ? (
                    <div className="empty-state">
                        <div style={{ width: '60px', height: '60px', backgroundColor: 'var(--bg-surface-muted)', borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px auto', color: 'var(--text-muted)' }}>
                            <ShoppingBag size={30} />
                        </div>
                        <p style={{ fontWeight: 500, color: 'var(--text-secondary)' }}>Keranjang pesanan Anda masih kosong.</p>
                        ...
                        <div key={item.menu_id} className="card" style={{ marginBottom: '14px', padding: '16px' }}>
                            <div style={{ display: 'flex', gap: '14px', marginBottom: '12px' }}>
                                {item.image_url
                                    ...
                                    : (
                                        <div style={{ width: '64px', height: '64px', backgroundColor: 'var(--bg-surface-muted)', borderRadius: 'var(--radius-lg)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)' }}>
                                            <Utensils size={24} />
                                        </div>
                                    )
                                }
                                ...
                                <button onClick={() => removeItem(item.menu_id)} style={{ background: 'none', color: 'var(--accent-red)', padding: '4px' }}>
                                    <Trash2 size={18} />
                                </button>
                            </div>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px', backgroundColor: 'var(--bg-surface-muted)', padding: '6px 14px', borderRadius: 'var(--radius-full)' }}>
                                <button className="btn btn-outline btn-sm" style={{ width: '28px', height: '28px', padding: 0, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => changeQty(item.menu_id, -1)}>
                                    <Minus size={14} />
                                </button>
                                <span style={{ fontWeight: 700, minWidth: '24px', textAlign: 'center' }}>{item.quantity}</span>
                                <button className="btn btn-primary btn-sm" style={{ width: '28px', height: '28px', padding: 0, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => changeQty(item.menu_id, 1)}>
                                    <Plus size={14} />
                                </button>
                            ...
                            <input
                                ...
                                placeholder="Catatan khusus (misal: ekstra pedas, es dikit)..."
                                value={item.note}
